...
def plot(t):
    ...
    for it in range(t):
        ...
        if judg is None:
            ...
            plt.title('Predicted u({},x,y)'.format(it))
            cbar_u = plt.colorbar()
            cbar_u.mappable.set_clim(-0.5, 0.5)
            plt.savefig('u_pred({})'.format(it))  # save
            # plt.show()
            plt.close()
            ...
            plt.title('Error u({},x,y)'.format(it))
            plt.savefig('u_error({})'.format(it))  # save
            # plt.show()
            plt.close()
...
def action_fig(num):
    ...
    with imageio.get_writer(uri='action_k_pred.gif', mode='I', fps=2) as writer:
        for i in range(num):
            writer.append_data(imageio.imread(f'k_pred({i}).png'))

    with imageio.get_writer(uri='action_u_pred.gif', mode='I', fps=2) as writer:
        for i in range(num):
            writer.append_data(imageio.imread(f'u_pred({i}).png'))

    with imageio.get_writer(uri='action_v_pred.gif', mode='I', fps=2) as writer:
        for i in range(num):
            writer.append_data(imageio.imread(f'v_pred({i}).png'))
